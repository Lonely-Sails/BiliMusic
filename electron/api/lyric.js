...
//  * B 站字幕 URL 有效期极短（约 10s），需要在获取 URL 后立即请求，
//  * 且不能使用缓存的 player/v2 响应（见 cache.js NO_CACHE_PATTERNS）。
//  * @param {string} bvid
//  * @param {number} cid
//  * @param {boolean} isRetry - 是否已经是重试
//  */
// async function getBilibiliSubtitle(bvid, cid, isRetry = false) {
//   const params = await sign({ bvid, cid })
//   const data = await apiGet('https://api.bilibili.com/x/player/v2', params, { skipCache: true })
//
//   if (data.code !== 0) return null
//   const playerData = data.data
//   if (!playerData?.subtitle?.subtitles?.length) return null
//
//   let subtitle = playerData.subtitle.subtitles.find(s => s.lan_doc === '中文')
//   if (!subtitle) subtitle = playerData.subtitle.subtitles[0]
//   if (!subtitle?.subtitle_url) return null
//
//   try {
//     const request = await fetch('https:' + subtitle.subtitle_url)
//     const subData = await request.json()
//     console.dir(subData, { depth: 2, colors: true })
//     return (subData.body || []).map((item) => ({
//       time: item.from,
//       text: normalizeText(item.content)
//     }))
//   } catch (err) {
//     // 如果第一次失败（可能是 URL 刚过期），重试一次获取新 URL
//     if (!isRetry) {
//       console.log('Subtitle fetch failed, retrying with fresh URL...')
//       return getBilibiliSubtitle(bvid, cid, true)
//     }
//     console.error('Subtitle fetch failed after retry:', err.message)
//     return null
//   }
// }
...
export {
  ...
  searchCandidates,
  fetchLyric,
  // getBilibiliSubtitle,  // B站字幕已禁用
  getOnlineLyric,
  searchRankedCandidates,
  // alignFirstLine,       // B站字幕校对已禁用
  // autoAlignAll,         // B站字幕校对已禁用
  // textSimilarity        // B站字幕校对已禁用
}
