<template>
  <div id="app-root">
    ...
    <div ref="containerRef" class="lyrics-container">
      <div v-if="!hasTrack" class="empty-text">暂无播放</div>
      <div v-else-if="lyrics.length === 0" class="empty-text">暂无歌词</div>
...
body {
  ...
  overflow: hidden;
  font-family:
    -apple-system, BlinkMacSystemFont, 'Segoe UI', 'PingFang SC', 'Microsoft YaHei', 'Noto Sans SC',
    sans-serif;
...
.lyric-line.active {
  ...
  transform: scale(1.1);
  text-shadow:
    0 0 12px rgba(251, 114, 153, 0.5),
    0 0 24px rgba(251, 114, 153, 0.15);
}
