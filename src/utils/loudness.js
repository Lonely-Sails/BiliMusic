...
export function getCachedGain(songKey) {
  return gainCache.get(songKey) ?? null;
}
...
export class LoudnessNormalizer {
  ...
  setUserVolume(userVolume) {
    this._lastVolume = userVolume;
    this._active ? this.fadeTo(userVolume * this._gain, 150) : this.fadeTo(userVolume, 150);
  }
  ...
  startAnalysis() {
    ...
    const tick = () => {
      if (!this._active || count >= ANALYSIS_SAMPLES) {
        this.finishAnalysis();
        return;
      }
  ...
  finishAnalysis() {
    ...
    // 峰值保护
    if (medianPeak * computedGain > 1.0) computedGain = 1.0 / medianPeak;
    this._gain = Math.max(MIN_GAIN, Math.min(MAX_GAIN, computedGain));
    if (this._songKey) gainCache.set(this._songKey, this._gain);
    console.log(
      `[Loudness] RMS=${medianRMS.toFixed(4)} Pk=${medianPeak.toFixed(4)} CF=${crestFactor.toFixed(1)} gain=${this._gain.toFixed(2)}x${this._songKey ? ' cached' : ''}`
    );
    this.applyGain();
  }
}
